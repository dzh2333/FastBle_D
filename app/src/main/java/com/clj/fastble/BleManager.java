...
public class BleManager {
    ...
    private final CopyOnWriteArrayList<Runnable> writeArrayList = new CopyOnWriteArrayList<>();
    private static volatile Runnable runnable = null;
    ...
    public synchronized void handlerMsg(){
        mHandler.postDelayed(() -> handlermsg(), 200);
    }
    private synchronized void handlermsg(){
        mHandler.removeCallbacksAndMessages(null);
        if (writeArrayList.size() > 0){
            if (runnable == null){
                synchronized (writeArrayList){
                    if (writeArrayList.size() > 0){
                        runnable = writeArrayList.get(0);
                        if (runnable != null){
                            runnable.run();
                        }else {
                            runnable = null;
    ...
    public void write(BleDevice bleDevice,
        ...
        writeArrayList.add(new Runnable() {
            @Override
            public void run() {
                write(bleDevice, uuid_service, uuid_write, data, true, new BleWriteCallback() {
                    @Override
                    ...
                    public void onWriteFailure(BleException exception) {
                        ...
                        //after disconnect to clear and intercept
                        runnable = null;
